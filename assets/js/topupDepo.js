...
app.controller('depositCtrl',function($scope, $rootScope, $http){
  var vm = this;
  ...
  $scope.filter = {};
    ...
    $scope.getTopupDepo = function(pageno) {
       $scope.dataLoadedSuccessfully = false;
       $scope.loaded = false;
      $http({
        method: 'POST',
        url: baseURL + "topupDepoListing",
        data :{num: $scope.itemsPerPage,
          page: pageno,
          filter: $scope.filter}
      }).then(function succes(e) {
        $scope.topup = e.data.tupupdepo;

        $scope.totalSaldo  = e.data.totalSaldo;
        $scope.totalAmount = e.data.totalAmount;
